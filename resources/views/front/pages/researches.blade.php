...
@section('title') Researches & Development @endsection
                        ...
                        <div class="row">
                            <div class="col-lg-5 col-md-12 col-pad">
                                <div class="property-thumbnail">
                                    <a href="{{ route('research_detail',['id'=>encrypt($research->id)]) }}" class="property-img">
                                        @if(!empty($research->image))
                                            <img src="{{ URL::asset(App\Models\research::DIR_STORAGE . $research->image) }}" alt="property-box-5" class="img-fluid">
                                        @else
                            ...
                            <div class="col-lg-7 col-md-12 align-self-center col-pad">
                                <div class="detail">
                                    <!-- title -->
                                    <h1 class="title">
                                        <a href="{{ route('research_detail',['id'=>encrypt($research->id)]) }}">{{ $research->title }}</a>
                                    </h1>
                                    @empty($research->location)

                                    @else
                                    <!-- Location -->
                                    <div class="location">
                                        <a href="{{ route('research_detail',['id'=>encrypt($research->id)]) }}">
                                            <i class="fa fa-map-marker"></i>{{ $research->location }}
                                        </a>
                                    </div>
                                    @endempty

                                    <!-- Paragraph -->
                                    <p>{{ $research->short_description }}</p>
                                    <p style="text-align: right;"><a href="{{ route('research_detail',['id'=>encrypt($research->id)]) }}" class="text-primary">Read More</a></p>
                                </div>
                            </div>
                        </div>
                    </div>
