...
@section('title') Awards & Recognitions @endsection
                        ...
                        <div class="row">
                            <div class="col-lg-5 col-md-12 col-pad">
                                <div class="property-thumbnail">
                                    <a href="{{ route('award_detail',['id'=>encrypt($award->id)]) }}" class="property-img">
                                        @if(!empty($award->image))
                                            <img src="{{ URL::asset(App\Models\Award::DIR_STORAGE . $award->image) }}" alt="property-box-5" class="img-fluid">
                                        @else
                            ...
                            <div class="col-lg-7 col-md-12 align-self-center col-pad">
                                <div class="detail">
                                    <!-- title -->
                                    <h1 class="title">
                                        <a href="{{ route('award_detail',['id'=>encrypt($award->id)]) }}">{{ $award->title }}</a>
                                    </h1>


                                    <!-- Paragraph -->
                                    <p>{{ $award->short_description }}</p>
                                    <p style="text-align: right;"><a href="{{ route('award_detail',['id'=>encrypt($award->id)]) }}" class="text-primary">Read More</a></p>
                                </div>
                            </div>
                        </div>
                    </div>
